...
-- Add RLS policies
ALTER TABLE state_controlled_substance_licenses ENABLE ROW LEVEL SECURITY;

-- Policy for authenticated users to read all records
CREATE POLICY "Allow authenticated users to read state_controlled_substance_licenses" 
ON state_controlled_substance_licenses FOR SELECT 
TO authenticated 
USING (true);

-- Policy for authenticated users to insert records
CREATE POLICY "Allow authenticated users to insert state_controlled_substance_licenses" 
ON state_controlled_substance_licenses FOR INSERT 
TO authenticated 
WITH CHECK (true);

-- Policy for authenticated users to update records
CREATE POLICY "Allow authenticated users to update state_controlled_substance_licenses" 
ON state_controlled_substance_licenses FOR UPDATE 
TO authenticated 
USING (true) 
WITH CHECK (true);

-- Policy for authenticated users to delete records
CREATE POLICY "Allow authenticated users to delete state_controlled_substance_licenses" 
ON state_controlled_substance_licenses FOR DELETE 
TO authenticated 
USING (true);
