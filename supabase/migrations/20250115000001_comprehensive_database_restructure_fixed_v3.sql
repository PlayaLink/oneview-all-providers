-- =====================================================
-- COMPREHENSIVE DATABASE RESTRUCTURE MIGRATION (FIXED V3 - SIMPLIFIED)
...
-- Handle both requirement_data and requirement_data_fields table names
DO $$
BEGIN
  -- Check if requirement_data_fields exists (from previous migration)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'requirement_data_fields') THEN
    CREATE TABLE IF NOT EXISTS facility_requirement_values (
      id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
      facility_id UUID NOT NULL REFERENCES facilities(id) ON DELETE CASCADE,
      requirement_id UUID NOT NULL REFERENCES requirements(id) ON DELETE CASCADE,
      requirement_data_id UUID NOT NULL REFERENCES requirement_data_fields(id) ON DELETE CASCADE,
...
-- Add missing columns to requirement_data or requirement_data_fields (if they don't exist)
DO $$
BEGIN
  -- Check if requirement_data_fields exists (from previous migration)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'requirement_data_fields') THEN
...
-- Requirement data indexes (handle both table names)
DO $$
BEGIN
  -- Check if requirement_data_fields exists (from previous migration)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'requirement_data_fields') THEN
...
-- Step 5: Create simple views for basic functionality
-- =====================================================
...
-- Simple facility properties view
...
-- Add updated_at triggers for all tables (only if they don't exist)
DO $$
BEGIN
  -- Check and create trigger for facility_property_values
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_facility_property_values_updated_at') THEN
    CREATE TRIGGER update_facility_property_values_updated_at 
      BEFORE UPDATE ON facility_property_values 
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  
  -- Check and create trigger for facility_providers
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_facility_providers_updated_at') THEN
    CREATE TRIGGER update_facility_providers_updated_at 
      BEFORE UPDATE ON facility_providers 
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  
  -- Check and create trigger for facility_requirements
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_facility_requirements_updated_at') THEN
    CREATE TRIGGER update_facility_requirements_updated_at 
      BEFORE UPDATE ON facility_requirements 
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  
  -- Check and create trigger for facility_requirement_values
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_facility_requirement_values_updated_at') THEN
    CREATE TRIGGER update_facility_requirement_values_updated_at 
      BEFORE UPDATE ON facility_requirement_values 
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  
  -- Check and create trigger for requirement_data or requirement_data_fields
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'requirement_data_fields') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_requirement_data_fields_updated_at') THEN
      CREATE TRIGGER update_requirement_data_fields_updated_at 
        BEFORE UPDATE ON requirement_data_fields 
        FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
  ELSE
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_requirement_data_updated_at') THEN
...
-- Grant permissions for requirement_data or requirement_data_fields
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'requirement_data_fields') THEN
...
-- Add comments for requirement_data or requirement_data_fields
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'requirement_data_fields') THEN
